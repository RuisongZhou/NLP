...
#加载训练好的model GoogleNews-vectors-negative300.bin

def load_binary_vec(name,vocab):
    word_vec = {}
    with open(name,'rb') as fn:
        ...
        for i in tqdm(range(vocab_size)):
            ...
            if word in vocab:
                word_vec[word] = np.fromstring(fn.read(binary_len), dtype=np.float32)
            else:
                fn.read(binary_len)
...
if  __name__ == '__main__':
    ...
    print ('size of word vector:' + len(list(word_vecs.keys())))
